<script>
	...
	$: topPerformers = (() => {
		...
		for (const ev of evaluationList || []) {
			...
			if (!key) continue;
			const current = latestByAthlete.get(key);
			const evTs = ts(ev?.date);
			const curTs = ts(current?.date);
			if (!current || evTs > curTs) {
				latestByAthlete.set(key, ev);
			}
		}
							...
							{#each topPerformers as performer, index (performer._id ?? performer.id ?? performer.date ?? performer.athlete)}
								<li>
									...
									<div class="name-block">
										<div class="title">{performer.athlete || performer.name || 'Unbekannt'}</div>
										<div class="sub">{performer.discipline || ''}</div>
									</div>
									<div class="pill-score">{performer.score ?? '-'}</div>
								</li>
							{/each}
						</ul>
					{/if}
